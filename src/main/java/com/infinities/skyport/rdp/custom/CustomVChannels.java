...
public class CustomVChannels {

	protected static Logger logger = LoggerFactory.getLogger(CustomVChannels.class);
	...
	public static final int CHANNEL_CHUNK_LENGTH = 1600;// the value is
														// 1600,don't change by
														// lijanaan 2009.12.14
	...
	private CustomVChannel channels[] = new CustomVChannel[MAX_CHANNELS];
	...
	public int mcs_id(int c) {
		return MCSUtils.MCS_GLOBAL_CHANNEL + 1 + c;
	}

	/**
	 * Initialise the maximum number of Virtual Channels
	 */
	public CustomVChannels(Options options) {
		channels = new CustomVChannel[MAX_CHANNELS];
	 ...
	 */
	public CustomVChannel channel(int c) {
		if (c < num_channels)
			return channels[c];
		else
			return null;
	 ...
	 */
	public CustomVChannel find_channel_by_channelno(int channelno) {
		if (channelno > MCSUtils.MCS_GLOBAL_CHANNEL + num_channels) {
			logger.warn("Channel " + channelno + " not defined. Highest channel defined is " + MCSUtils.MCS_GLOBAL_CHANNEL
					+ num_channels);
			return null;
		} else
			return channels[channelno - MCSUtils.MCS_GLOBAL_CHANNEL - 1];
	}
	 ...
	 */
	public boolean register(CustomVChannel v) throws RdesktopException {
		...
		v.set_mcs_id(MCSUtils.MCS_GLOBAL_CHANNEL + 1 + num_channels);
		num_channels++;
	 ...
	 */
	public void channel_process(RdpPacket_Localised data, int mcsChannel, RDPSession session, ChannelHandlerContext context)
			throws RdesktopException, IOException, CryptoException {

		int length, flags;
		// int thislength = 0;
		CustomVChannel channel = null;
		...
		if (((flags & CHANNEL_FLAG_FIRST) != 0) && ((flags & CHANNEL_FLAG_LAST) != 0)) {
			...
			channel.process(data, session, context);
		} else {
			...
			if ((flags & CHANNEL_FLAG_LAST) != 0) {
				...
				channel.process(fullpacket, session, context);
