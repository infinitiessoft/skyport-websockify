 ...
 */
public class AvailablePortFinder implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public final static String MIN_PORT_NUMBER_STR = "Min_Port";
	public final static String MAX_PORT_NUMBER_STR = "Max_Port";
	private final static Logger logger = LoggerFactory.getLogger(AvailablePortFinder.class);

	/**
	 * The minimum number of server port number.
	 */
	private int MIN_PORT_NUMBER = 1;

	/**
	 * The maximum number of server port number.
	 */
	private int MAX_PORT_NUMBER = 49151;


	/**
	 * Creates a new instance.
	 */
	public AvailablePortFinder(int min, int max) {
		checkState(max > min, "max port number should greater than min port number");
		checkState(max > 0, "max port number should greater than zero");
		checkState(max < 65535, "max port number should less than zero");
		checkState(min > 0, "min port number should greater than zero");
		MIN_PORT_NUMBER = min;
		MAX_PORT_NUMBER = max;
	}
	...
	public boolean available(int port) {
		...
		try {
			...
			logger.debug("port: {} occupied", port);
		} finally {
			if (ds != null) {
				ds.close();
			}
	 ...
	 */
	public Set<Integer> getAvailablePorts(int fromPort, int toPort) {
