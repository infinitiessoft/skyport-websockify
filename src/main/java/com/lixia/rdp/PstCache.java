...
import com.infinities.skyport.rdp.custom.CustomOrdersPanel;

public class PstCache {

	protected static Logger logger = LoggerFactory.getLogger(PstCache.class);
	...
	private CustomOrdersPanel ordersPanel;
	...
	public int getG_stamp() {
		return g_stamp;
	}

	public void setG_stamp(int g_stamp) {
		this.g_stamp = g_stamp;
	}

	public File[] getG_pstcache_fd() {
		return g_pstcache_fd;
	}

	public void setG_pstcache_fd(File[] g_pstcache_fd) {
		this.g_pstcache_fd = g_pstcache_fd;
	}

	public int getG_pstcache_Bpp() {
		return g_pstcache_Bpp;
	}

	public void setG_pstcache_Bpp(int g_pstcache_Bpp) {
		this.g_pstcache_Bpp = g_pstcache_Bpp;
	}

	public boolean isG_pstcache_enumerated() {
		return g_pstcache_enumerated;
	}

	public void setG_pstcache_enumerated(boolean g_pstcache_enumerated) {
		this.g_pstcache_enumerated = g_pstcache_enumerated;
	}

	public PstCache(Options options, CustomOrdersPanel ordersPanel) {
		...
		try {
			...
			if (fd != null) {
				try {
					...
					logger.warn("close bitmap stream failed", e);
				}
			}
		}
	}

	private byte[] toBigEndian32(int value) {
		byte[] out = new byte[4];
	...
	public boolean pstcache_load_bitmap(int cache_id, int cache_idx) throws IOException, RdesktopException {
		...
		if (!options.isPersistent_bitmap_caching())
			return false;
		...
		try {
			...
			bitmap = new Bitmap(options, celldata, c.width, c.height, 0, 0, options.getBpp());
			// bitmap = ui_create_bitmap(cellhdr.width, cellhdr.height,
			// celldata);
			ordersPanel.getCache().putBitmap(cache_id, cache_idx, bitmap, c.stamp);
			// xfree(celldata);
			return true;
		} finally {
			if (fd != null) {
	...
	public boolean pstcache_put_bitmap(int cache_id, int cache_idx, byte[] bitmap_id, int width, int height, int length,
		...
		try {
			fd = new FileOutputStream(g_pstcache_fd[cache_id]);
			int offset = cache_idx * (options.getBpp() * MAX_CELL_SIZE + CELLHEADER.size());
	...
	public int pstcache_enumerate(int cache_id, int[] idlist) throws IOException, RdesktopException {
		...
		if (!(options.isBitmap_caching() && options.isPersistent_bitmap_caching() && IS_PERSISTENT(cache_id)))
			return 0;
		...
		for (n = 0; n < BMPCACHE2_NUM_PSTCELLS; n++) {
			try {
				...
				if (result != 0) {
					...
					if (cellhdr.stamp != 0) {
						/*
						 * Pre-caching is not possible with 8bpp because a
						 * colourmap is needed to load them
						 */
						if (options.isPrecache_bitmaps() && (options.getServer_bpp() > 8)) {
	...
	public boolean pstcache_init(int cache_id) {
		...
		if (!(options.isBitmap_caching() && options.isPersistent_bitmap_caching()))
			return false;

		g_pstcache_Bpp = options.getBpp();
