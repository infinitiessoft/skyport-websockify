...
public class Bitmap {
	...
	private static Logger logger = LoggerFactory.getLogger(Bitmap.class);


	public static int convertTo24(Options options, int colour) {
		logger.debug("server_bpp: {}", options.getServer_bpp());
		if (options.getServer_bpp() == 15)
			return convert15to24(colour);
		if (options.getServer_bpp() == 16)
	...
	public static int convert16to24(int colour16) {
		...
		g24 |= g24 >> 6;
		b24 |= b24 >> 5;
		logger.debug("r:{}, g:{}, b:{}", new Object[] { r24, g24, b24 });
		return (r24 << 16) | (g24 << 8) | b24;
	}

	public static long covert16to32(int colour16) {
		logger.debug("colour16: {}", colour16);
		long r = (colour16 & 0xF800) >> 11;
		long g = (colour16 & 0x07E0) >> 5;
		long b = (colour16 & 0x001F);

		r = r * 255 / 31;
		g = g * 255 / 63;
		b = b * 255 / 31;
		
		logger.debug("16to32 r:{}, g:{}, b:{}", new Object[] { r, g, b });
		
		return (0xFF << 24) | (r) | (g << 8) | (b << 16);
	}

	public int convert24to16(int colour24) {
		return (((colour24 >> 8) & 0xF800) | ((colour24 >> 5) & 0x07E0) | ((colour24 >> 3) & 0x001F));
	}
	 ...
	 * @return
	 */
	public static int cvalx(Options options, byte[] data, int offset, int Bpp) {
		int rv = 0;
		if (options.getServer_bpp() == 15) {
		...
		} else if (options.getServer_bpp() == 16) {
			int lower = data[offset] & 0xFF;
			int full = (data[offset + 1] & 0xFF) << 8 | lower;
	 ...
	 * @return
	 */
	public static int getli(byte[] input, int startOffset, int offset, int Bpp) {
		int rv = 0;
	 ...
	 * @param Bpp
	 */
	public static void setli(byte[] input, int startlocation, int offset, int value, int Bpp) {
	 ...
	 */
	public static int[] convertImage(Options options, byte[] bitmap, int Bpp) {
		...
		for (int i = 0; i < out.length; i++) {
			if (Bpp == 1)
				...
				out[i] = ((bitmap[i * Bpp + 2] & 0xFF) << 16) | ((bitmap[i * Bpp + 1] & 0xFF) << 8)
						| (bitmap[i * Bpp] & 0xFF);
			out[i] = convertTo24(options, out[i]);
		}
		return out;
	}
	 ...
	 */
	public Bitmap(Options options, int[] data, int width, int height, int x, int y) {
	...
	public Bitmap(Options options, byte[] data, int width, int height, int x, int y, int Bpp) {
		this.highdata = convertImage(options, data, Bpp);
		this.width = width;
		this.height = height;
		this.x = x;
		this.y = y;
	 ...
	 */
//	public static WrappedImage decompressImgDirect(Options options, ChannelHandlerContext context, int width, int height,
//			int size, RdpPacket_Localised data, int Bpp, IndexColorModel cm, int left, int top, WrappedImage w)
//			throws RdesktopException {
//
//		// WrappedImage w = null;
//
...
//						if (previous == -1) {
//							// pixel[line+x] = mix;
//							w.setRGB(context, left + x, top + height, mix);
//						} else {
//							w.setRGB(context, left + x, top + height, w.getRGB(left + x, top + prevY) ^ mix);
...
//							for (int i = 0; i < 8; i++) {
//								// pixel[line+x] = 0;
//								w.setRGB(context, left + x, top + height, 0);
//								count--;
//								x++;
//							}
//						}
...
//								w.setRGB(context, left + x, top + height, w.getRGB(left + x, top + prevY));
//								count--;
//								x++;
//							}
//						}
...
//									w.setRGB(context, left + x, top + height, (byte) mix);
//								} else {
//									// pixel[line + x] = 0;
//									w.setRGB(context, left + x, top + height, 0);
//								}
...
//									// pixel[line + x] = (pixel[previous + x] ^
//									// mix);
//									w.setRGB(context, left + x, top + height, w.getRGB(left + x, prevY + top) ^ mix);
//								} else {
//									// pixel[line + x] = pixel[previous + x];
//									w.setRGB(context, left + x, top + height, w.getRGB(left + x, prevY + top));
//								}
//								count--;
//								x++;
//							}
...
//						for (int i = 0; i < 8; i++) {
//							// pixel[line + x] = color2;
//							w.setRGB(context, left + x, top + height, color2);
//							count--;
//							x++;
//						}
//					}
...
//							// Bpp);
//							w.setRGB(context, left + x, top + height, cvalx(options, compressed_pixel, input, Bpp));
//							input += Bpp;
//							count--;
//							x++;
//						}
...
//							} else {
//								// pixel[line + x] = color1;
//								w.setRGB(context, left + x, top + height, color1);
//								bicolor = true;
//								count++;
//							}
//							count--;
...
//							w.setRGB(context, left + x, top + height, 0xffffff);
//							count--;
//							x++;
//						}
//					}
...
//						for (int i = 0; i < 8; i++) {
//							// pixel[line + x] = 0x00;
//							w.setRGB(context, left + x, top + height, 0x00);
//							count--;
//							x++;
//						}
//					}
...
//	}
	 ...
	 * @throws RdesktopException
	 */
//	public static Image decompressImg(Options options, ChannelHandlerContext context, int width, int height, int size,
//			RdpPacket_Localised data, int Bpp, IndexColorModel cm) throws RdesktopException {
//
//		WrappedImage w = null;
//
//		byte[] compressed_pixel = new byte[size];
...
//						if (previous == -1) {
//							// pixel[line+x] = mix;
//							w.setRGB(context, x, height, mix);
//						} else {
//							w.setRGB(context, x, height, w.getRGB(x, prevY) ^ mix);
//							// pixel[line+x] = (pixel[previous+x] ^ mix);
//						}
//
//						insertmix = false;
...
//							for (int i = 0; i < 8; i++) {
//								// pixel[line + x] = pixel[previous + x];
//								w.setRGB(context, x, height, w.getRGB(x, prevY));
//								count--;
//								x++;
//							}
//						}
...
//								if ((mask & mixmask) != 0) {
//									// pixel[line + x] = (byte) mix;
//									w.setRGB(context, x, height, (byte) mix);
//								} else {
//									// pixel[line + x] = 0;
//									w.setRGB(context, x, height, 0);
//								}
...
//						for (int i = 0; i < 8; i++) {
//							// pixel[line + x] = color2;
//							w.setRGB(context, x, height, color2);
//							count--;
//							x++;
//						}
//					}
...
//							// pixel[line + x] = cvalx(compressed_pixel, input,
//							// Bpp);
//							w.setRGB(context, x, height, cvalx(options, compressed_pixel, input, Bpp));
//							input += Bpp;
//							count--;
//							x++;
//						}
...
//							} else {
//								// pixel[line + x] = color1;
//								w.setRGB(context, x, height, color1);
//								bicolor = true;
//								count++;
//							}
//							count--;
...
//						for (int i = 0; i < 8; i++) {
//							// pixel[line + x] = 0xffffff;
//							w.setRGB(context, x, height, 0xffffff);
//							count--;
//							x++;
//						}
//					}
...
//						for (int i = 0; i < 8; i++) {
//							// pixel[line + x] = 0x00;
//							w.setRGB(context, x, height, 0x00);
//							count--;
//							x++;
//						}
//					}
...
//	}
	 ...
	 * @throws RdesktopException
	 */
	public static int[] decompressInt(Options options, int width, int height, int size, RdpPacket_Localised data, int Bpp)
			throws RdesktopException {
		...
		int mask = 0;
		int mix = 0xffffffff;
		// logger.debug("opcode origin: {}", opcode);
		boolean insertmix = false, bicolor = false, isfillormix = false;
		...
		while (input < end) {
			...
			// logger.debug("opcode updated: {}", opcode);
			switch (opcode) {
			case 0: /* Fill */
				if ((lastopcode == opcode) && !((x == width) && (previous == -1)))
					insertmix = true;
	...
	public int[] bitmap_decompress(int width, int height, int size, RdpPacket_Localised data, int Bpp)
			throws RdesktopException {
		int[] ret = null;
		switch (Bpp) {
		case 1:
			ret = decompressInt(options, width, height, size, data, Bpp);
			break;
		case 2:
		case 3:
		case 4:
	...
	public int[] decompressInt3(int width, int height, int size, RdpPacket_Localised data, int Bpp) throws RdesktopException {
	 ...
	 * @throws RdesktopException
	 */
	public static byte[] decompress(int width, int height, int size, RdpPacket_Localised data, int Bpp)
			throws RdesktopException {
