...
public abstract class CustomKeyPair {
	...
	public static CustomKeyPair genKeyPair(int type) throws JSchException {
		return genKeyPair(type, 1024);
	}

	public static CustomKeyPair genKeyPair(int type, int key_size) throws JSchException {
		CustomKeyPair kpair = null;
		if (type == DSA) {
			kpair = new CustomKeyPairDSA();
		} else if (type == RSA) {
			kpair = new CustomKeyPairRSA();
		} else if (type == ECDSA) {
			kpair = new CustomKeyPairECDSA();
		}
		if (kpair != null) {
	...
	private Cipher genCipher() {
		try {
			Class<?> c;
			c = Class.forName(Config.getConfig("3des-cbc"));
	...
	public static CustomKeyPair load(String prvkey) throws JSchException {
	...
	public static CustomKeyPair load(String prvfile, String pubfile) throws JSchException {
	...
	public static CustomKeyPair load(byte[] prvkey, byte[] pubkey) throws JSchException {
		...
		if (pubkey == null
				&& prvkey != null
				&& (prvkey.length > 11 && prvkey[0] == 0 && prvkey[1] == 0 && prvkey[2] == 0 && (prvkey[3] == 7 || prvkey[3] == 19))) {
		...
		try {
			...
			while (i < len) {
				if (buf[i] == 'B' && i + 3 < len && buf[i + 1] == 'E' && buf[i + 2] == 'G' && buf[i + 3] == 'I') {
					...
					} else if (i + 6 < len && buf[i] == 'P' && buf[i + 1] == 'R' && buf[i + 2] == 'I' && buf[i + 3] == 'V'
							&& buf[i + 4] == 'A' && buf[i + 5] == 'T' && buf[i + 6] == 'E') {
						type = UNKNOWN;
						vendor = VENDOR_PKCS8;
						encrypted = false;
						i += 3;
					} else if (i + 8 < len && buf[i] == 'E' && buf[i + 1] == 'N' && buf[i + 2] == 'C' && buf[i + 3] == 'R'
							&& buf[i + 4] == 'Y' && buf[i + 5] == 'P' && buf[i + 6] == 'T' && buf[i + 7] == 'E'
							&& buf[i + 8] == 'D') {
						type = UNKNOWN;
						vendor = VENDOR_PKCS8;
						i += 5;
				...
				if (buf[i] == 'A' && i + 7 < len && buf[i + 1] == 'E' && buf[i + 2] == 'S' && buf[i + 3] == '-'
					...
					i += 8;
					if (Session.checkCipher(Config.getConfig("aes256-cbc"))) {
						Class<?> c = Class.forName(Config.getConfig("aes256-cbc"));
						cipher = (Cipher) (c.newInstance());
				...
				if (buf[i] == 'A' && i + 7 < len && buf[i + 1] == 'E' && buf[i + 2] == 'S' && buf[i + 3] == '-'
					...
					i += 8;
					if (Session.checkCipher(Config.getConfig("aes192-cbc"))) {
						Class<?> c = Class.forName(Config.getConfig("aes192-cbc"));
						cipher = (Cipher) (c.newInstance());
						// key=new byte[cipher.getBlockSize()];
						iv = new byte[cipher.getIVSize()];
					} else {
				...
				if (buf[i] == 'A' && i + 7 < len && buf[i + 1] == 'E' && buf[i + 2] == 'S' && buf[i + 3] == '-'
					...
					i += 8;
					if (Session.checkCipher(Config.getConfig("aes128-cbc"))) {
						Class<?> c = Class.forName(Config.getConfig("aes128-cbc"));
						cipher = (Cipher) (c.newInstance());
						// key=new byte[cipher.getBlockSize()];
						iv = new byte[cipher.getIVSize()];
					} else {
	...
	static CustomKeyPair loadPPK(byte[] buf) throws JSchException {
		byte[] pubkey = null;
		byte[] prvkey = null;
		int lines = 0;
		...
		if (typ.equals("ssh-rsa")) {
			...
			kpair = new CustomKeyPairRSA(n_array, pub_array, null);
			...
			kpair = new CustomKeyPairDSA(p_array, q_array, g_array, y_array, null);
		} else {
			return null;
		}
